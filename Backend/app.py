...
def create_order():
    ...
    try:
        ...
        print("thisssssssss",type(course_amount) ,course_amount)
        # print(client)
        # print(client.order)
        # print(course_amount)
        # print(course_amount*100)
        
        course_amount= round(course_amount*100, 1)
...
@app.route('/subscriptionValidity',methods=['POST'])
    ...
    try:
        ...
        if recharge_type == 'wallet' and wallet is not None:
            ...
            if contentId is not None:
                ...
                #here we 
                wallet.balance =round(wallet.balance+Decimal(money),1)
                if wallet.balance >= price:
                    ...
                    save_payment_details(payment_details)
                    db.session.commit()
                    return jsonify({"message": "wallet recharge successful and content price deducted", 'payment_details': payment_details})
                else:
                    db.session.commit()
                    return jsonify({"message":"Low wallet balance"}),200
            else:
                print("walltet balance type",type(wallet.balance))
                wallet.balance =round(wallet.balance+Decimal(money),1)
...
#             s_id=1
#             accesslevel={"access":"all","period":"monthly"}
...
#             end_day=start_day+(5*60)
#             s_id=5
#             accesslevel={"access":"all","period":"yearly"}
...
#             if wallet:
#                 price=0
#                 #this is for article  price fetch
#                 article=get_article_by_id(contentId)
#                 if article!=None:
#                     price=article['price']
...
#                 #   now update the wallet balance with money
#                 wallet.balance = wallet.balance+money
#                 #if the content price (price)is greter than  current updated value then content price will be deducted  from wallet and save the details in payment details and wallet details
#                 if wallet.balance>=price:
...
@app.route('/get_articleCost_by_id', methods=['POST'])
...
def pay_article():
    ...
    wallet = Wallet.query.filter_by(user_id=data['uid']).first()
    if wallet and wallet.balance >= content.price:
        wallet.balance =wallet.balance-content.price
        payment_details={
        "user_id":uid,
        "order_id":wallet_order_id,
        "payment_id":'',
