...
import { useState, useEffect, arateContext, createName } from "react";
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  useEffect(() => {
    console.log(localStorage.getItem("uid"),"this is uid")
    checkSessionStatus()
  }, []);
...
// this function for whether the user have valid subscription or not
    ...
    const handleViewClick = (isPremium) => {
        ...
        if (isPremium) {
            ...
            });//all navigation will redirct to content page
            }
             else if(payPerContentPrice >0 && uid!=null ){
  ...
  const handlePay = async () => {
    if (walletBalance<payPerContentPrice) {
        ...
        navigate('/payment-option', {
          state: { contentPrice: payPerContentPrice ,articleId:contentId}
        });
    }
    else{
      try {
    ...
    return (
        <>
            <div>
              ...
              <h1><img src={mathrubhumiIcon} alt="" /></h1>
    ...
    {isLoggedIn ? (
      ...
      <div onClick={() => navigate("/login")}>Login</div>
                    ...
                    {isLoggedIn &&(<div className={classes["subscription"]}>
                      <button className={classes["suscribe-button"]} onClick={handleSubscriptionButton}>Take Subscription</button>
                    </div>)}
                        ...
                        {articles.length > 0 && (() => {
                            ...
                            for (let i = 0; i < articles.length; i++) {
                                ...
                                articleElements.push(
                                    <div className={classes['article']} key={article.id} onMouseOver={() => handleChange(article.price,article.id)} >
                                        ...
                                        <button className={classes['news-btn']} onClick={() => handleViewClick(true)}>View</button>
                        ...
                        <div className={classes['popup']}>
                            <div className={classes['popup-content']}>
                                <div className={classes['close']}>
                                    <button className={classes['close-btn']} onClick={closePopup}>X</button>
                                </div>
